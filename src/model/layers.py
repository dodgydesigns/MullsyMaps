...
class AnnotationsLayer:
    ...
    def updateZoom(self, scale):
        ...
        for layer in self.view.annotationLayers.layers.values():
            ...
            tl = self.view.mapController.toCanvasCoordinates(layer.lat, layer.lon) - QPointF(layer.width / 2,
                                                                                             layer.height / 2)
...
class SpirographLayer:
    ...
    def __init__(self, view):
        ...
        self.view = view
        self.controller = view.mapController
    ...
    def fill(self):
        ...
        for spiro in self.spirographList:
            ...
            for segment in spiro.spiroSegmentList:
                segmentCentreXY = self.controller.toCanvasCoordinates(segment.segmentCentreLatLon.x(),
                                                                      segment.segmentCentreLatLon.y())
                segment.soundSpeed = ncwmsTools.getSoundSpeedForPoint(self.depth, segmentCentreXY.x(),
                                                                      segmentCentreXY.y())
    ...
    def update(self):
        """
        Called when the map is zoomed. The Spirographs are redrawn using the sound speed determined
        when they were created. This saves having to repeat the expensive call to the ncMWS server. """
        for spiro in self.spirographList:
    ...
    def addProbOfDetectionBoundaries(self):
        ...
        for lineSegment in self.view.rulerLayer.lineSegmentList:
            ...
            while (lengthCount * SONAR_RANGE_OF_THE_DAY * 0.5) < dist:
                rangeOfTheDayRadiusLatLon = self.controller.vincentyDirect(lineSegment.startLatLon.x(),
...
class NavChartsTemplateLayer():
    ...
    def __init__(self, view):
        ...
        self.chartOutlines['AUS00111P0'] = (
            115.67178994467334, -32.255049939198756, 115.72476925338074, -32.18514286687904)
        self.chartOutlines['AUS00111P1'] = (
            115.67398200849902, -32.18514991811797, 115.68981583872339, -32.17214993911571)
        self.chartOutlines['AUS00114P1'] = (
            115.73333022129219, -32.26388987184614, 115.7600043579987, -32.24222204006769)
        self.chartOutlines['AUS00114P0'] = (
            115.69994738308844, -32.24694949354819, 115.7867245026005, -32.1305528125873)
        self.chartOutlines['AUS00113P0'] = (
            115.68164733325914, -32.072674243475596, 115.76385406845286, -32.02849897906623)
        self.chartOutlines['AUS00117P0'] = (
            115.59978316828143, -32.28336893515372, 115.77857152703865, -32.04998801900059)
        self.chartOutlines['AUS00112P0'] = (
            115.4259400743493, -32.078144013356045, 115.83690500989755, -31.857167891674703)

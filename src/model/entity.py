...
def correctOrientation(angle, tlx=-1, tly=1):
...
class Entity(UIObject):
    ...
    def __init__(self, view, designation, affiliation, x, y, speed, course, rnge, bearing, classification, certainty):
        ...
        self.iconCenterX = 0
        self.iconCenterY = 0
        self.certaintyCircleCircleX = 0
        self.certaintyCircleCircleY = 0
        self.x = 0
        self.y = 0
        self.lat = 0
        self.lon = 0
    ...
    def draw(self):
        ...
        x, y = correctOrientation(self.course, lineX - self.icon.pos().x(), lineY - self.icon.pos().y())
        t.translate(x, y)
        ...
        courseLine = self.view.scene.addLine(QLineF(lineX,
                                                    lineY,
                                                    (lineX + sin(radians(
                                                        self.course)) * self.speed * enums.PREDICTED_COURSE_SCALE),
                                                    (lineY - cos(radians(
                                                        self.course)) * self.speed * enums.PREDICTED_COURSE_SCALE)))
    ...
    def zoom(self):
        """
        Scale the entity icon using Qt. All graphicsLayers scale automatically but hide/show graphicsLayers

...
from src.gis.mts_controller import vincentyDirect
...
class Ownship(UIObject):
    ...
    def drawSpiroPolygons(self):
        ...
        courseLine = scene.addLine(QLineF(lineX,
                                          lineY,
                                          (lineX + sin(radians(
                                              self.course)) * self.speed * preferences.PREDICTED_COURSE_SCALE),
                                          (lineY - cos(radians(
                                              self.course)) * self.speed * preferences.PREDICTED_COURSE_SCALE)))
        ...
        fivekydsLatLon = vincentyDirect(self.lat, self.lon, 0.0, 4572)  # 4572m = 5 kyds
        fivekydsInPixels = self.view.mapController.toCanvasCoordinates(fivekydsLatLon.x(), fivekydsLatLon.y()).y()

        layer = self.graphicsLayers[preferences.DISTANCE_MARKERS]
        layer.addGraphicsObject(DistanceMarker(name='fiveKy',
                                               proxy=scene.addEllipse(
                                                   0, 0,
                                                   preferences.FIVE_KY_RADIUS,
                                                   preferences.FIVE_KY_RADIUS),
                                               ...
                                               pen=QPen(Qt.red,
                                                        preferences.CIRCLE_PEN)))

        layer.addGraphicsObject(DistanceMarker(name='tenKy',
                                               proxy=scene.addEllipse(
                                                   0, 0,
                                                   preferences.TEN_KY_RADIUS,
                                                   preferences.TEN_KY_RADIUS),
        ...
        layer.addGraphicsObject(DistanceMarker(name='fifteenKy',
                                               proxy=scene.addEllipse(
                                                   0, 0,
                                                   preferences.FIFTEEN_KY_RADIUS,
                                                   preferences.FIFTEEN_KY_RADIUS),
        ...
        layer.addGraphicsObject(DistanceMarker(name='twentyKy',
                                               proxy=scene.addEllipse(
                                                   0, 0,
                                                   preferences.TWENTY_KY_RADIUS,
                                                   preferences.TWENTY_KY_RADIUS),
        ...
        layer.addGraphicsObject(DistanceMarker(name='twentyFiveKy',
                                               proxy=scene.addEllipse(
                                                   0, 0,
                                                   preferences.TWENTYFIVE_KY_RADIUS,
                                                   preferences.TWENTYFIVE_KY_RADIUS),
    ...
    def update(self, x, y, speed, course):
        ...
        fivekydsLatLon = vincentyDirect(self.lat, self.lon, 0.0, 5468)  # 4572m = 5kyds
        fivekydsInPixels = self.view.mapController.toCanvasCoordinates(fivekydsLatLon.x(),
                                                                       fivekydsLatLon.y()).y() - self.y
    ...
    def zoom(self):
        """
        Scale the entity icon using Qt. All graphicsLayers scale automatically but hide/show
        graphicsLayers at certain zoom levels.
        """
